...
public sealed class BlogRepository : IBlogRepository
{
    ...
    public async Task AddAsync(Blog blog, CancellationToken cancellationToken)
    {
        ...
        if (blog.Author is not null)
        {
            ...
            {
                // Handle User navigation property to avoid tracking conflicts
                if (blog.Author.User is not null)
                {
                    var trackedUser = _dbContext.ChangeTracker
                        .Entries<Domain.Entities.ApplicationUser>()
                        .FirstOrDefault(entry => entry.Entity.Id == blog.Author.User.Id);

                    if (trackedUser is not null && !ReferenceEquals(blog.Author.User, trackedUser.Entity))
                    {
                        // User is already tracked with a different instance, use the tracked instance
                        var authorEntry = _dbContext.Entry(blog.Author);
                        authorEntry.Reference(a => a.User).CurrentValue = trackedUser.Entity;
                    }
                }
