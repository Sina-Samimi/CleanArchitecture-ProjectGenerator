...
public sealed record UpdateTeacherProductCommand(
    Guid ProductId,
    string TeacherId,
    string Name,
    string? Summary,
    ...
    string? DigitalDownloadPath) : ICommand<bool>
{
    public sealed class Handler : ICommandHandler<UpdateTeacherProductCommand, bool>
    {
        ...
        public async Task<Result<bool>> Handle(UpdateTeacherProductCommand request, CancellationToken cancellationToken)
        {
            ...
            var slug = product.SeoSlug;
            if (!string.Equals(product.Name, normalizedName, StringComparison.Ordinal))
            {
                slug = await GenerateUniqueSlugAsync(normalizedName, product.Id, cancellationToken);
            }
        ...
        private async Task<string> GenerateUniqueSlugAsync(string name, Guid productId, CancellationToken cancellationToken)
        {
            ...
            while (await _productRepository.ExistsBySlugAsync(slug, productId, cancellationToken))
            {
