...
namespace Arsis.Domain.Entities.Billing;
...
public sealed class Invoice : Entity, IAggregateRoot
{
    ...
    {
        var item = new InvoiceItem(this, name, description, itemType, referenceId, quantity, unitPrice, discountAmount);
    ...
    public void ResetItems(IEnumerable<(string Name, string? Description, InvoiceItemType Type, Guid? ReferenceId, decimal Quantity, decimal UnitPrice, decimal? DiscountAmount, IReadOnlyCollection<(string Key, string Value)>? Attributes)> items)
    {
        ...
        {
            AddItem(item.Name, item.Description, item.Type, item.ReferenceId, item.Quantity, item.UnitPrice, item.DiscountAmount, item.Attributes);
        }
