...
using EndPoint.WebSite.Models.Account;
...
public sealed class AccountController : Controller
{
    ...
    public async Task<IActionResult> SendCode(PhoneLoginViewModel model, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(normalizedPhone) || normalizedPhone.Length != 11 || !normalizedPhone.StartsWith('0'))
        {
            ModelState.AddModelError(nameof(model.PhoneNumber), "لطفاً شماره موبایل معتبر وارد کنید.");
        }
    ...
    private static string NormalizePhoneNumber(string? phoneNumber)
    {
        ...
        foreach (var character in phoneNumber)
        {
            ...
            {
                var numeric = (int)char.GetNumericValue(character);
                if (numeric >= 0)
                {
    ...
    {
        TempData["Alert.Title"] = "ورود ناموفق";
        TempData["Alert.Message"] = "مشکلی در ورود شما وجود دارد. لطفاً با پشتیبانی تماس بگیرید.";
