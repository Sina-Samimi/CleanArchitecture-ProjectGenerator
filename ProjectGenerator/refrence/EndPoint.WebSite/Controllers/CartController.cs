...
public sealed class CartController : Controller
{
    ...
    public async Task<IActionResult> Add(Guid productId, int quantity = 1, string? returnUrl = null, CancellationToken cancellationToken = default)
    {
        ...
        var result = await _mediator.Send(new AddCartItemCommand(userId, anonymousId, productId, quantity <= 0 ? 1 : quantity), cancellationToken);
        if (!result.IsSuccess || result.Value is null)
        {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> UpdateQuantity(Guid productId, int quantity, CancellationToken cancellationToken)
    {
        var userId = GetUserId();
        var anonymousId = _cartCookieService.GetAnonymousCartId();

        var result = await _mediator.Send(new UpdateCartItemQuantityCommand(userId, anonymousId, productId, quantity), cancellationToken);
        if (!result.IsSuccess || result.Value is null)
        {
    ...
    public async Task<IActionResult> RemoveItem(Guid productId, CancellationToken cancellationToken)
    {
        ...
        var result = await _mediator.Send(new RemoveCartItemCommand(userId, anonymousId, productId), cancellationToken);
        if (!result.IsSuccess || result.Value is null)
        {
