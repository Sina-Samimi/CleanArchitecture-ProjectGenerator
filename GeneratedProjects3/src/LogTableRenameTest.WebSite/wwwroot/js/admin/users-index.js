        ...
        function bindUserForm(root) {
            ...
            form.addEventListener('submit', async (event) => {
                ...
                if (phoneInput) {
                    ...
                    const phonePattern = /^(?:(?:0?9)|(?:\+?989))\d{2}[\s\-]?\d{3}[\s\-]?\d{4}$/;
                    if (!phonePattern.test(phoneRaw)) {
                        // show inline error if present
                        const errorElem = phoneInput.parentElement ? phoneInput.parentElement.querySelector('.text-danger, .field-validation-error') : null;
                        if (errorElem) {
                            errorElem.textContent = 'شماره موبایل معتبر وارد کنید.';
                        } else {
