...
namespace LogTableRenameTest.Application.Commands.Identity.DeleteUser;

public sealed record SoftDeleteUserCommand(string UserId) : ICommand<UserDto>
{
    public sealed class Handler : ICommandHandler<SoftDeleteUserCommand, UserDto>
    {
        ...
        public async Task<Result<UserDto>> Handle(SoftDeleteUserCommand request, CancellationToken cancellationToken)
        {
            ...
            if (user.IsDeleted)
            {
                return Result<UserDto>.Failure($"User '{user.Email}' is already deleted.");
            }
            ...
            {
                user.IsActive = false;
                user.DeactivatedOn = user.DeactivatedOn ?? user.DeletedOn;
            }
